...
/*   Updated: 2025/05/05 11:06:35 by maelmahf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	handle_stars(char **pattern, char **last_wildcard,char **last_match, char *str)
{
    while (**pattern == '*')
        (*pattern)++;
    if (!**pattern)
...
bool    pattern_matches(char *pattern , char *str)
{
    ...
    while(*str)
    {
        ...
        if(*pattern == '*' && !quotes && handle_stars(&pattern, &last_wildcard, &last_match, str))
            return (true);
        else if(!pattern_match_exists(&pattern, &last_wildcard, &last_match, &str))
            return (false);
    }
    if (*pattern == '*')
        while (*pattern == '*')
